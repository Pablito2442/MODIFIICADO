...
ej_ejecutar_comprobar_memoria(){
    ...
    if [ ${#colaMemoria[@]} -gt 0 ]; then
        ...
        tiempoRestante=${tREj[${colaEjecucion[0]}]}

        # Obtener el tiempo de ejecución del primer proceso en la cola de memoria
        tiempoCola=${tiempoEjecucion[${colaMemoria[0]}]}
...
ej_ordenar_cola_memoria() {
    ...
    for (( i=0; i<${#colaMemoria[@]}; i++ )); do
        for (( j=$i+1; j<${#colaMemoria[@]}; j++ )); do
            ...
            if [[ ${tiempoEjecucion[${colaMemoria[$i]}]+_} && ${tiempoEjecucion[${colaMemoria[$j]}]+_} ]]; then
                if [ ${tiempoEjecucion[${colaMemoria[$i]}]} -gt ${tiempoEjecucion[${colaMemoria[$j]}]} ]; then
                    tmp=${colaMemoria[$i]}
...
ej_pantalla_cola_ejecucion() {
    ...
    echo -n -e " Cola Ejecucion (Orden ejecución por FIFO):"
...
ej_pantalla_cola_memoria(){
    ...
    echo -n -e " Cola Memoria(Orden entrada en memoria por SRPT):"
