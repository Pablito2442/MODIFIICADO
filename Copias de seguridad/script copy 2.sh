...
ej_ejecutar_comprobar_reubicacion() {
    ...
    for (( mar=0; mar <= $numeroMarcos; mar++ ));do
        ...
        # Si el marco no está vacío o se ha llegado al final de la memoria
            ...
            # Si se alcanza el mínimo de marcos para la reubicaion y el contador es distinto de 0
...
ej_ejecutar_sacar_ejecucion(){
    ...
    #Decrementa 1 ya que se ejecuta el proceso del marco 
    (( --tREj[$enEjecucion] ))

    # AQUI HYA QUE MODIFICAR O CREAR UNA NUEVA FUNCION PARA QUE GUARDE EL PROGRESO ANTES DE FINALIZAR LOS FALLOS DE PAGINACION.

    #ESTO TAL VEZ LO TENGAS QUE QUITAR DESPUES
    ej_ejecutar_guardar_fallos

    # Resetear los marcos actuales
    guardar_marcosActuales=${marcosActuales[*]}
    marcosActuales=()
...
ej_ejecutar_guardar_fallos() {
    ...
    if [[ $algoritmo_srtp -ne 1 ]];then
        for mar in ${!guardar_marcosActuales[*]};do
            marco=${guardar_marcosActuales[$mar]}
...
ej_ejecutar() {
    ...
    if [[ -n "$enEjecucion"  &&  ${estado[enEjecucion]} -eq 3 ]];then

        # Ejecucion del algoritmo e introduce el siguiente proceso en caso de se cumplan las condiciones
        ej_ejecutar_comprobar_algoritmo_SRTP \
            &&  ej_ejecutar_sacar_ejecucion \
                &&  ej_ejecutar_empezar_ejecucion \
                    && (( ++tREj[$enEjecucion] )) \
