...
ej_pantalla_fin_fallos() {
    ...
    local anchoBloque=$anchoMomento
    ...
    # Luego tienes que quitar este comentario para hacerla local y no global
    ...
    for (( l=0; ; l++ ));do
        ...
        for (( mar=0; mar<${minimoEstructural[$fin]}; mar++ ));do
            ...
            for (( mom=$primerMomento; mom<=$ultimoMomento; mom++ ));do
                # Introducida pagina en la tabla
                if [ ${marcoFallo[$mom]} -eq $mar ];then
                    printf "${cf[3]}╔%${anchoGen}s╗${cf[0]}" "${resumenFallos[$mom,$mar]}"

                # # Muestra el apuntador del proceso.
                elif [[ ${marcoFallo[$mom]} -eq $((mar-1)) ]];then

                #     # Caso de que el puntero esta bien situado saliendo en rojo
                #     if [ ${numFallos[$enEjecucion]} -eq $numFallosAntes ];then
                        printf "${cf[4]}┌%${anchoGen}s┐${cf[0]}" "${resumenFallos[$mom,$mar]}"
                #         [ $numFallosAntes -ne 0 ] \
                #             && ((--numFallosAntes))

                #     # MODIFICAR TODAVIA TIENE QUE PONER EN CASO DE QUE SEA EL ULTIMO MARCO EL APUNTADOR EN LA PRIMERA CASILLA     
                #     elif [[ ${minimoEstructural[$fin]} -eq $((mar+1)) ]];then
                #         printf "${cf[7]}┌%${anchoGen}s┐${cf[0]}" "${resumenFallos[$mom,$mar]}"


                #     # Apuntador mal posicionado y de color naranja para diferenciarlo
                #     else
                #         printf "${cf[6]}┌%${anchoGen}s┐${cf[0]}" "${resumenFallos[$((mom-1)),$mar]}"
                #     fi
            ...
            for (( mom=$primerMomento; mom<=$ultimoMomento; mom++ ));do
                if [[ ${marcoFallo[$mom]} -eq $mar ]];then
                    ...
                    printf "${cf[4]}└%${anchoMomento}s┘${cf[0]}" "${resumenFIFO[$mom,$mar]}"
                else
...
ej_pantalla_informacion() {
    ...
    echo -e "El numero de mom en este caso es de: $mom"
    echo -e "El numero de mom por definciion en este caso es de: $(( ${pc[$enEjecucion]} - 1 ))"
    echo -e "El numero de SRPT en este caso es de: ${algoritmo_srtp[$enEjecucion]}"
    echo -e "El numero anterior de SRPT en este caso es de: "
